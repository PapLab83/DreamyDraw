...
def build_stage1_2_graph(
    *,
    ...
    graph.add_node(r.NODE_INPUT_ANALYSIS, _persisting(stage1.input_analysis, storage))
    graph.add_node(r.NODE_METADATA_LOOKUP, _persisting(partial(stage1.metadata_lookup, registry=registry), storage))
    graph.add_node(r.NODE_REQUEST_CLASSIFICATION, _persisting(stage1.request_classification, storage))
    graph.add_node(r.NODE_CLARIFICATION_INTERRUPT, _persisting(stage1.clarification_interrupt, storage))
    graph.add_node(r.NODE_EMPTY_INPUT_INTERRUPT, _persisting(stage1.empty_input_interrupt, storage))
    graph.add_node(r.NODE_UNSUPPORTED_INTERRUPT_OR_STOP, _persisting(stage1.unsupported_interrupt_or_stop, storage))
    graph.add_node(
        r.NODE_CANDIDATE_LAYER_RESOLUTION,
        _persisting(partial(stage1.candidate_layer_resolution, registry=registry), storage),
    )
    graph.add_node(
        r.NODE_FINAL_PARAMETER_VALIDATION,
        _persisting(partial(stage1.final_parameter_validation, registry=registry), storage),
    )
    graph.add_node(r.NODE_PREVIEW, _persisting(stage1.preview, storage))
    graph.add_node(
        r.NODE_PROMPT_CONTEXT_PREPARATION,
        _persisting(partial(stage1.prompt_context_preparation, registry=registry, composer=composer), storage),
    )
    graph.add_node(
    ...
    graph.add_node(
        r.NODE_SCORER,
        _persisting(partial(stage2.scorer, registry=registry, composer=composer, text_executor=text_executor), storage),
    )
    graph.add_node(r.NODE_RANKER, _persisting(stage2.ranker, storage))
    graph.add_node(
    ...
    graph.add_node(r.NODE_APPROVED_TEXT_SELECTOR, _persisting(stage2.approved_text_selector, storage))
    graph.add_node(r.NODE_SHORTAGE_FALLBACK_INTERRUPT, _persisting(shortage_fallback_interrupt, storage))
...
def _persisting(node: Callable[[GraphState], GraphState], storage: JSONStorage | None):

...
- **Цель:** Сгенерировать пул из `IDEA_POOL_SIZE` идей в правильном жанровом регистре + `global_context` (описание персонажа).
- **Выход:** `session.ideas_pool` (`IDEA_POOL_SIZE` объектов `Idea`), `session.global_context`.
- **DoD:** Пул из `IDEA_POOL_SIZE` валидных идей в нужном режиме.
...
|---|---|
| от 1 до `USER_ARBITRATION_THRESHOLD - 1` | Переход к `plan_needs_refine`. Авторежим: ревьюер и редактор работают самостоятельно, без участия пользователя. |
| `USER_ARBITRATION_THRESHOLD` и выше | Переход к `plan_needs_user_arbitration`. Пользователь видит проблемные темы и может вмешаться. |
| > `MAX_VALIDATION_RETRIES` | Принудительный переход в `failed`. Защита от бесконечной петли. |

### Арбитраж пользователя

Когда `validation_cycles >= USER_ARBITRATION_THRESHOLD`, в `main.py` показывается:

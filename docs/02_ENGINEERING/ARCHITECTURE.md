...
User Request -> [Safety Gate] -> [Config Match]
      |
[Series Planner] -> Генерирует настраиваемый пул идей (`IDEA_POOL_SIZE`)
      |
...
- **Санитарный фильтр:** идеи со скором ниже `MIN_CHILD_INDEX` отсекаются сразу.
- **Взвешенная рулетка:** шансы выбора пропорциональны качеству идеи, но сглажены `SCORE_NORMALIZATION_EPSILON` для сохранения разнообразия.
- **Конфигурация:** все числовые пороги пайплайна должны храниться в `settings.py` или `constants.py`. Подробная инвентаризация: `docs/02_ENGINEERING/CONFIGURATION_CONSTANTS.md`.

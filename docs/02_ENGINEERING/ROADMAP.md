...
- [x] дополнительно
           - Нужно поменять промпт генерации идей по мотивам пользователя - сейчас он откровено странные идеи дает и там бы хотелось получать интересные факты про животных в целом то что интересно и взросылм просто это можно будет переделать на детский лад
           - Валидация уже готовых текстов историй перед показом пользователю или давать возможность пользователю вносить изменения по темам (вместо строгого ответа согласия или нет с текстом)
           - Ассинхронный запск иллюстраций
           - все магические числа и коде и в промтах вынести в конфиг 
           - сделать умной необходимость пользователю вмешиваться в спор между валидатором и редактором (например после 5 попыток или когда валидатор получает низкий рейтинг от 1 до 100 по идее) 

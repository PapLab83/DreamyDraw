...
| Сказочные истории про лису для 5 лет | `story`, `FAIRY_TALE`, `NARRATIVE`, age `5`, subject fox | `CONTENT_FORMAT_STORY`, `FAIRY_TALE_BASE`, `UTILITY_NARRATIVE_BASE`, `AGE_5`, `LANGUAGE_RU_AUDIENCE`, `LANGUAGE_RU_RESULT`, `RUSSIAN_FOLK_TALE` or `SCANDINAVIAN_TALE`, `FAIRY_TALE_ANIMAL_FOX` | none expected | fairy-tale behavior allowed; age 5 complexity |
| Мифологическая мягкая история про солнце или ветер для 5 лет | `story`, `MYTH`, `NARRATIVE`, age `5`, nature subject | `CONTENT_FORMAT_STORY`, `MYTH_BASE`, `UTILITY_NARRATIVE_BASE`, `AGE_5`, `LANGUAGE_RU_AUDIENCE`, `LANGUAGE_RU_RESULT`, `MYTH_SOFT_BASE` | sun/wind may remain freeform nature subject unless a future entity layer is added | mythic explanation is gentle, not factual science claim |
| Поучительная правдивая история про мытьё рук после прогулки | `story`, `TRUTH`, `TEACHING`, topic handwashing, age default or extracted | `CONTENT_FORMAT_STORY`, `TRUTH_BASE`, `UTILITY_TEACHING_BASE`, `UTILITY_TOPIC_HAND_WASHING_AFTER_WALK`, `AGE_3` or `AGE_5`, `LANGUAGE_RU_AUDIENCE`, `LANGUAGE_RU_RESULT`, `ENTITY_SOAP`, `ENTITY_HANDS`, optionally `ENTITY_CHILD` / `ENTITY_CARING_ADULT` | walk context can be hard detail | `utility_goal` critical; hygiene advice correct |
